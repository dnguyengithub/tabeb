...
Where each column corresponds to a feature and reach row corresponds to an observation. The columns can take data type of text, number, or even images or audio (these two modalities are not yet supported in this version). 

And tabular embedding model (or tabular encoder) encode each row of the table to a real-valued vector of fixed size *emb_size*. 

TabEB evaluates the performance of tabular encoders based on the performance of down stream models that using the embedding generated by them. Sepecifically, we identified the following downstream tasks and the corresponding evaluation protocols:
...
| Unsupervised anomaly detection | Given a table, find the abnormal rows (outlier, duplicates, etc.) | - Fraud detection- Equipment maintenance: detecting unusual sensor readings in machinery that could signal potential failures. | Both |  | Weighted (micro) F1 score. |
...
- The choice of linear regression and logistic regression for Regression and Classification tasks, resp. can be controversial, since tree-based models still outperform deep learning on tabular data ([https://arxiv.org/abs/2207.08815](https://arxiv.org/abs/2207.08815)). Our argument for choosing linear regression and logistic regression is the linear transformation may reflect better the quality of the embedding. However, these choices are subject to chance in the future.
- The pertinence of the evaluation metric depends on the specific context of the downstream task. We choose the most common one for each task.
...
from tabeb.interface import get_model
...
# Run the tasks
df_regression_scores = task_regression.evaluate(model_list, save_dir=os.path.join(ROOT_DIR, "results"))
df_classification_scores = task_classification.evaluate(model_list, save_dir=os.path.join(ROOT_DIR, "results"))
